...
for (var _i = 0, _a = ['input', 'change']; _i < _a.length; _i++) {
    ...
    document.querySelector('#us_iterations').addEventListener(event_name_1, function () {
        us_sankey.iterations = +this.value;
        us_sankey.redraw();
    });
    // Set layout algorithm alpha
    document.querySelector('#us_alpha').addEventListener(event_name_1, function () {
    ...
    document.getElementById('us_link_path_curvature').addEventListener(event_name_1, function () {
        us_sankey.link_path_curvature = +this.value;
        us_sankey.redraw();
    });
}
// Change node width when changing between percent or pixel based units.
for (var _b = 0, _c = document.forms['us_sankey'].elements['us_node_width_type']; _b < _c.length; _b++) {
    var radio = _c[_b];
...
for (var _d = 0, _e = document.forms['us_sankey'].elements['us_node_padding_type']; _d < _e.length; _d++) {
    var radio = _e[_d];
    radio.addEventListener('change', function () {
        var event = document.createEvent('HTMLEvents');
        event.initEvent('change', true, true);
...
for (var _f = 0, _g = document.forms['us_sankey'].elements['us_node_label_orientation']; _f < _g.length; _f++) {
    var radio = _g[_f];
    radio.addEventListener('change', function () {
...
// IE doesn't support `input` events
for (var _h = 0, _j = ['input', 'change']; _h < _j.length; _h++) {
    var event_name_2 = _j[_h];
    // Set layout algorithm iterations
    document.querySelector('#uk_iterations').addEventListener(event_name_2, function () {
        uk_sankey.iterations = +this.value;
        uk_sankey.redraw();
    });
    // Set layout algorithm alpha
    document.querySelector('#uk_alpha').addEventListener(event_name_2, function () {
    ...
    // Set link path curvature
    document.getElementById('uk_link_path_curvature').addEventListener(event_name_2, function () {
        uk_sankey.link_path_curvature = +this.value;
        uk_sankey.redraw();
    });
}
// Change node width when changing between percent or pixel based units.
for (var _k = 0, _l = document.forms['uk_sankey'].elements['uk_node_width_type']; _k < _l.length; _k++) {
    var radio = _l[_k];
    radio.addEventListener('change', function () {
        var event = document.createEvent('HTMLEvents');
        event.initEvent('change', true, true);
...
// Change node padding when changing between percent or pixel based units.
for (var _m = 0, _o = document.forms['uk_sankey'].elements['uk_node_padding_type']; _m < _o.length; _m++) {
    var radio = _o[_m];
    radio.addEventListener('change', function () {
        var event = document.createEvent('HTMLEvents');
        event.initEvent('change', true, true);
