...
var zoom_example = new c3.Plot.Zoomable({
    ...
    h: d3.scale.linear().domain([-10, 10]),
    v: d3.scale.linear().domain([-50, 80]),
...
$('#zoom_parabolic_button').click(function () {
    zoom_line_layer.y = function (d) { return d * d; };
    zoom_line_layer.interpolate = 'basis';
    zoom_example.redraw();
});
...
$('#zoom_bell_button').click(function () {
    zoom_example.layers.unshift(new c3.Plot.Layer.Area({
        y: function (d) { return 500 / (Math.sqrt(variance) * Math.sqrt(2 * Math.PI)) *
