...
var stacked_bar_chart = new c3.Plot({
    height: 300,
...
$('#stack_example_plots').append(stacked_bar_chart.anchor);
...
d3.tsv("data/injury_cause.tsv")
    ...
    if (error)
        alert("AJAX error downloading CDC data: " + error.statusText);
    // Filter invalid rows from the CDC's tsv file.
    data = data.filter(function (row) { return !!row.cause; });
