...
(function() {
  ...
  c3.Polar = (function(_super) {
    __extends(Polar, _super);

    function Polar() {
      this.toPolar = __bind(this.toPolar, this);
    ...
    Polar.prototype.toPolar = function(x, y) {
      var radius, theta, _ref;
      _ref = c3.Polar.toPolar(x, y), theta = _ref[0], radius = _ref[1];
  ...
  c3.Polar.Layer.Radial = (function(_super) {
    ...
    Radial.prototype.outer_radius = Infinity;
    ...
    Radial.prototype._init = function() {
      ...
      if (this.draggable) {
        ...
        this.dragger.on('drag', function(d, i) {
          var depth, _ref;
          _ref = self.toPolar(d3.event.x, d3.event.y), drag_value = _ref[0], depth = _ref[1];
    ...
    Radial.prototype._draw = function(origin) {
      ...
      this.lines.animate(origin === 'redraw' || origin === 'rebase').position(line_position = {
        ...
        y2: (function(_this) {
          return function(d, i) {
            var r;
            return _this.r((r = outer_radius(d, i)) !== Infinity ? r : window.innerHeight + window.innerWidth);
          };
        })(this)
      });
      if (this.draggable) {
  ...
  c3.Polar.Layer.Segment = (function(_super) {
    __extends(Segment, _super);

    function Segment() {
      this.get_position_from_key = __bind(this.get_position_from_key, this);
      this._style = __bind(this._style, this);
      this._draw = __bind(this._draw, this);
      this._update = __bind(this._update, this);
      this._init = __bind(this._init, this);
    ...
    Segment.prototype._init = function() {
      ...
      if (this.arc_options.animate) {
        if ((_base = this.arc_options).animate_old == null) {
          _base.animate_old = true;
        }
      }
    ...
    Segment.prototype._draw = function(origin) {
      var new_r_domain, new_t_domain, prev_t_domain, r_interpolation, root_node, t_interpolation, _ref, _ref1;
      if (this.root_nodes != null) {
        root_node = (_ref = this.root_node) != null ? _ref : {
          x1: 0,
          x2: 1,
          y1: 0
        };
        prev_t_domain = (_ref1 = (origin !== 'rebase' ? this.prev_t_domain : void 0)) != null ? _ref1 : this.t.domain();
      ...
      this.arcs.animate(origin === 'redraw' || origin === 'revalue' || origin === 'rebase').position_tweens({
        'd': (function(_this) {
          return function(d) {
            var arc_interpolation, node, _ref2, _ref3, _ref4, _ref5;
            node = _this.nodes[_this.key(d)];
            arc_interpolation = d3.interpolateObject({
              x1: (_ref2 = node.px1) != null ? _ref2 : node.x1,
              x2: (_ref3 = node.px2) != null ? _ref3 : node.x2,
              y1: (_ref4 = node.py1) != null ? _ref4 : node.y1,
              y2: (_ref5 = node.py2) != null ? _ref5 : node.y2
            }, node);
            return function(t) {
    ...
    Segment.prototype.get_position_from_key = function(key) {
      var _ref;
      return (_ref = this.nodes) != null ? _ref[key] : void 0;
    };
  ...
  c3.Polar.Layer.Arc = (function(_super) {
    ...
    Arc.prototype._layout = function(data) {
      var d, end_angle, i, inner_radius, key, node, outer_radius, start_angle, _i, _len;
  ...
  c3.Polar.Layer.Pie = (function(_super) {
    ...
    Pie.prototype._layout = function(data) {
      var angle, d, delta, inner_radius, key, node, outer_radius, total, _i, _j, _len, _len1;
      inner_radius = c3.functor(this.inner_radius);
      outer_radius = c3.functor(this.outer_radius);
      total = 0;
      ...
      if (this.other_options) {
        ...
        if (this.data.length > this.limit_elements && this.sort === true) {
          return this.other_arc.singleton().update().position_tweens({
            'd': (function(_this) {
              return function(d, i) {
                var interpolate, other_node, _ref;
                other_node = {
                ...
                interpolate = d3.interpolateObject((_ref = _this.prev_other_node) != null ? _ref : other_node, other_node);
    ...
    Pie.prototype._style = function(style_new) {
      ...
      return (_ref = this.other_arc) != null ? _ref.style(style_new) : void 0;
    };
  ...
  c3.Polar.Layer.Sunburst = (function(_super) {
    __extends(Sunburst, _super);

    function Sunburst() {
      this.get_leaf = __bind(this.get_leaf, this);
      this._style = __bind(this._style, this);
      this._draw = __bind(this._draw, this);
      this._update = __bind(this._update, this);
      this.rebase_key = __bind(this.rebase_key, this);
    ...
    Sunburst.prototype._init = function() {
      ...
      if ((_base = this.arc_options).events == null) {
        _base.events = {};
      }
      if ((_base1 = this.arc_options.events).click == null) {
        _base1.click = (function(_this) {
          return function(d) {
      ...
      if ((_base2 = this.bullseye_options).events == null) {
        _base2.events = {};
      }
      if ((_base3 = this.bullseye_options.events).click == null) {
        _base3.click = (function(_this) {
          return function() {
            return _this.rebase(null);
          };
        })(this);
      }
      if ((_base4 = this.bullseye_options).title == null) {
        _base4.title = "Navigate to root of tree";
      }
      return this.center = this.content.select('circle.center').singleton();
    };

    Sunburst.prototype._layout = function(data, origin) {
      var build_nodes, collect_nodes, compute_values, current_data, datum, key, limit_angle_percentage, node, nodes, old_node, old_nodes, parent_key, parent_node, partition, root_domain, set_depth, sort, value, _i, _j, _k, _l, _len, _len1, _len2, _len3, _name, _ref, _ref1, _ref2, _ref3, _ref4, _ref5, _ref6;
      if (origin !== 'revalue' && origin !== 'rebase') {
        ...
        if (this.parent_key != null) {
          ...
          for (_i = 0, _len = data.length; _i < _len; _i++) {
            datum = data[_i];
            node = nodes[_name = this.key(datum)] != null ? nodes[_name] : nodes[_name] = {
              children: []
            };
            node.datum = datum;
          ...
          set_depth = (function(_this) {
            return function(node, depth) {
              var child, _j, _len1, _ref, _results;
              node.y1 = depth;
              node.y2 = depth + 1;
              _ref = node.children;
              _results = [];
              for (_j = 0, _len1 = _ref.length; _j < _len1; _j++) {
                child = _ref[_j];
                _results.push(set_depth(child, depth + 1));
              }
              return _results;
            };
          })(this);
          _ref = this.root_nodes;
          for (_j = 0, _len1 = _ref.length; _j < _len1; _j++) {
            node = _ref[_j];
            set_depth(node, 0);
          }
            ...
            return function(datum, depth) {
              ...
              node = nodes[_this.key(datum)] = {
                ...
                children: _this.children != null ? (function() {
                  ...
                  _ref2 = (_ref1 = this.children(datum)) != null ? _ref1 : [];
                  _results = [];
                  for (_k = 0, _len2 = _ref2.length; _k < _len2; _k++) {
                    child = _ref2[_k];
                    _results.push(build_nodes(child, depth + 1));
                  }
                  return _results;
                }).call(_this) : []
              };
          ...
          this.root_nodes = (function() {
            ...
            for (_k = 0, _len2 = data.length; _k < _len2; _k++) {
              datum = data[_k];
              _results.push(build_nodes(datum, 0));
            }
            return _results;
          })();
        }
        if (this.arc_options.animate) {
          for (key = _k = 0, _len2 = nodes.length; _k < _len2; key = ++_k) {
            node = nodes[key];
      ...
      partition = (function(_this) {
        return function(nodes, domain, total) {
          var angle, delta, dx, start, _len4, _len5, _m, _n;
      ...
      current_data = [];
      root_domain = [(_ref3 = (_ref4 = this.root_node) != null ? _ref4.x1 : void 0) != null ? _ref3 : 0, (_ref5 = (_ref6 = this.root_node) != null ? _ref6.x2 : void 0) != null ? _ref5 : 1];
      collect_nodes = function(nodes) {
        ...
        for (_m = 0, _len4 = nodes.length; _m < _len4; _m++) {
          ...
          if (node.x2 - node.x1 > limit_angle_percentage && node.x2 > root_domain[0] && node.x1 < root_domain[1]) {
            ...
            if (node.children.length) {
              _results.push(collect_nodes(node.children));
            } else {
    ...
    Sunburst.prototype.rebase_key = function(key) {
      ...
      this.trigger('rebase_start', (_ref = this.root_node) != null ? _ref.datum : void 0);
      this.chart.redraw('rebase');
      return this.trigger('rebase', (_ref1 = this.root_node) != null ? _ref1.datum : void 0);
    };
    ...
    Sunburst.prototype._draw = function(origin) {
      ...
      if (origin !== 'rebase') {
        return this.center.animate(origin === 'redraw').position({
          r: Math.max(0, this.r((_ref = (_ref1 = this.root_node) != null ? _ref1.y1 : void 0) != null ? _ref : 0))
        });
      }
    };
